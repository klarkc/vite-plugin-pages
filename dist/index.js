...
async function getPageFiles(path, options) {
  ...
  const cwds = await _fastglob2.default.call(void 0, path, {
    ignore,
    onlyDirectories: true,
    dot: true,
    unique: true
  });
  const nestedFiles = await Promise.all(cwds.map((cwd) => _fastglob2.default.call(void 0, `**/*.${ext}`, {
    ignore,
    onlyFiles: true,
    cwd
  })));
...
function prepareRoutes(routes, options, pagesDirOptions, parent) {
  ...
  for (const route of routes) {
    ...
    const filePath = slash(_path.join.call(void 0, options.root, route.component));
